...
for (const currency of ['SEK', 'EUR', 'USD']) {
  test(`PayPal POS ${currency} export, price update, checkout, automatic credit and concurrent replay`, async ({
    page,
  }) => {
    ...
    try {
      ...
      const configured = await page.request.post(
        'http://127.0.0.1:3456/_test/currency',
        {
          headers: { authorization: 'Bearer fixture:' + f.tenant },
          data: { currency },
        },
      )
      ...
      await page
        .getByLabel(`${d.sales.vatModes.consignment_margin} (%)`, {
          exact: true,
        })
        .fill('0')
      await page
      ...
      const products = async () => {
        const r = await page.request.get(
          'http://127.0.0.1:3456/_test/products',
          {
      ...
      await page
        .getByRole('button', { name: d.zettle.sync, exact: true })
        .click()
      const retryDone = page.waitForResponse(
        (r) =>
          r.url().endsWith('/api/integrations/zettle') && r.status() === 200,
      )
      await page
      ...
      expect(
        exported.every((p) => p.variants[0].price.currencyId === currency),
      ).toBe(true)
      ...
      expect(
        exported.every(
          (p) =>
            p.variants[0].price.amount === 20000 &&
      ...
      expect(
        exported.find((p) => p.uuid === first.uuid)?.variants[0].uuid,
      ).toBe(first.variants[0].uuid)
      ...
      await page
        .getByRole('link', { name: d.zettle.older, exact: true })
        .click()
      await expect(
        page.getByRole('link', { name: new RegExp(d.zettle.open) }),
      ).toHaveCount(2)
