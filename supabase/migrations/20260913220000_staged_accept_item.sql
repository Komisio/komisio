-- P1 S4 (staged): an agent may propose commercial acceptance; a person decides.
-- Risk medium: the approver must be a different identity than the proposer.
-- Follows docs/STAGED-OPERATIONS.md "Adding a kind": one private validate,
-- preflight and execute function, one line per dispatcher, additive only.
alter table public.pending_operations drop constraint pending_operations_kind_check;
alter table public.pending_operations add constraint pending_operations_kind_check check(kind in ('publishReceptionReview','saveInspectionDraft','acceptItem'));
...
create function komisio_private.op_validate_accept_item(value jsonb) returns boolean
language plpgsql immutable set search_path='' as $$
begin
 if value is null or jsonb_typeof(value)<>'object' then return false; end if;
 if not(value ?& array['originKind','originId','originRevision','priceOre']) or (value-array['originKind','originId','originRevision','priceOre'])<>'{}'::jsonb then return false; end if;
 if jsonb_typeof(value->'originKind')<>'string' or value->>'originKind' not in ('inspection_draft','reception_review','purchase') then return false; end if;
 if jsonb_typeof(value->'originId')<>'string' or (value->>'originId') !~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' then return false; end if;
 if (value->>'originKind'='purchase')<>(jsonb_typeof(value->'originRevision')='null') then return false; end if;
 if jsonb_typeof(value->'originRevision')='number' and ((value->>'originRevision') !~ '^[1-9][0-9]{0,9}$' or (value->>'originRevision')::bigint>2147483646) then return false; end if;
 if jsonb_typeof(value->'originRevision') not in ('null','number') then return false; end if;
 if jsonb_typeof(value->'priceOre')<>'number' or (value->>'priceOre') !~ '^[1-9][0-9]{0,10}$' or (value->>'priceOre')::bigint>99999999999 then return false; end if;
 return true;
end $$;

-- The cheap, current-state preconditions. Execution runs accept_item, which
-- re-evaluates everything (agreement prerequisite, seller mode, terms).
create function komisio_private.op_preflight_accept_item(p_tenant uuid,p_payload jsonb) returns jsonb
language plpgsql security definer set search_path='' as $$
declare kind text:=p_payload->>'originKind'; oid uuid:=(p_payload->>'originId')::uuid; rev integer:=(p_payload->>'originRevision')::integer; latest integer;
begin
 if exists(select 1 from public.items where tenant_id=p_tenant and origin_kind=kind and origin_id=oid) then raise exception 'ITEM_EXISTS'; end if;
 case kind
 when 'inspection_draft' then
  select revision into latest from public.inspection_draft_revisions where tenant_id=p_tenant and draft_id=oid order by revision desc limit 1;
  if latest is null then raise exception 'ORIGIN_NOT_FOUND'; end if;
  if latest<>rev then raise exception 'INSPECTION_DRAFT_CHANGED'; end if;
  if (select archived from public.inspection_draft_revisions where tenant_id=p_tenant and draft_id=oid and revision=latest) then raise exception 'INSPECTION_ARCHIVED'; end if;
 when 'reception_review' then
  select version into latest from public.reception_reviews where tenant_id=p_tenant and session_id=oid order by version desc limit 1;
  if latest is null then raise exception 'ORIGIN_NOT_FOUND'; end if;
  if latest<>rev then raise exception 'RECEPTION_REVIEW_CHANGED'; end if;
  if not exists(select 1 from public.garment_receipts where tenant_id=p_tenant and session_id=oid) then raise exception 'CUSTODY_REQUIRED'; end if;
 when 'purchase' then
  if not exists(select 1 from public.purchase_receipts where tenant_id=p_tenant and id=oid) then raise exception 'ORIGIN_NOT_FOUND'; end if;
 end case;
 return jsonb_build_object('origin_kind',kind,'origin_id',oid);
end $$;

create function komisio_private.op_execute_accept_item(p_tenant uuid,p_operation uuid,p_payload jsonb) returns uuid
language plpgsql security definer set search_path='' as $$
begin
 return public.accept_item(p_tenant,p_operation,p_payload->>'originKind',(p_payload->>'originId')::uuid,(p_payload->>'originRevision')::integer,(p_payload->>'priceOre')::bigint);
end $$;
revoke all on function komisio_private.op_validate_accept_item(jsonb),komisio_private.op_preflight_accept_item(uuid,jsonb),komisio_private.op_execute_accept_item(uuid,uuid,jsonb) from public,anon,authenticated;
...
begin
 ...
 if (p_kind is not null and p_kind not in ('publishReceptionReview','saveInspectionDraft','acceptItem'))
