-- Allow photo-only drafts to become empty without deleting evidence or files.
...
begin
 ...
 if jsonb_array_length(value) not between 0 and 20 then return false; end if;
...
begin
 ...
 -- An empty draft only detaches existing photos, never clears written evidence.
 if jsonb_array_length(p_sources)=0 and (coalesce(latest,0)=0 or not exists(
  select 1 from public.reception_source_revisions r where r.session_id=p_session and r.revision=latest
   and jsonb_array_length(r.sources)>0
   and not exists(select 1 from jsonb_array_elements(r.sources) s where s->>'kind'<>'photo')
