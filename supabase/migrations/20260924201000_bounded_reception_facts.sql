-- Keep normal queue pages bounded inside the shared query. The snapshot alone
-- requests all facts; an outer LIMIT on a PL/pgSQL set function is too late.
drop function public.reception_queue_facts(uuid,text,timestamptz,uuid);
create or replace function public.reception_queue_facts(p_tenant uuid,p_stage text default null,p_before timestamptz default null,p_before_id uuid default null,p_limit integer default null)
...
revoke all on function public.reception_queue_facts(uuid,text,timestamptz,uuid,integer) from public,anon;
grant execute on function public.reception_queue_facts(uuid,text,timestamptz,uuid,integer) to authenticated;
...
language sql stable security invoker set search_path='' as $$
 select * from public.reception_queue_facts(p_tenant,p_stage,p_before,p_before_id,21)
