...
select throws_like($$select propose_operation(current_setting('test.tenant')::uuid,gen_random_uuid(),'recordZettlePurchase',(select p from payload),'zettle-fixture',now()+interval '1 day')$$,'%ZETTLE_UNMATCHED_LINES%','no partial purchase');
...
select throws_like($$select resolve_zettle_line(current_setting('test.tenant')::uuid,gen_random_uuid(),current_setting('test.import')::uuid,2,1,current_setting('test.item2')::uuid)$$,'%ZETTLE_MATCH_CHANGED%','stale mapping rejected');
select is((select count(*) from sales),0::bigint,'resolution creates no sale');
select set_config('test.op',gen_random_uuid()::text,true);
select propose_operation(current_setting('test.tenant')::uuid,current_setting('test.op')::uuid,'recordZettlePurchase',(select p from payload),'zettle-fixture',now()+interval '1 day');
select is((select risk_level from pending_operations where id=current_setting('test.op')::uuid),'medium','financial import is medium risk');
select throws_ok($$select decide_operation(current_setting('test.tenant')::uuid,gen_random_uuid(),current_setting('test.op')::uuid,'approved','')$$,'42501',null,'proposer cannot approve');
-- Even choosing the same item again changes the reviewed version.
select resolve_zettle_line(current_setting('test.tenant')::uuid,gen_random_uuid(),current_setting('test.import')::uuid,2,2,current_setting('test.item2')::uuid);
reset role;
...
select is((select error_code from operation_decisions where operation_id=current_setting('test.op')::uuid),'ZETTLE_MATCH_CHANGED','approval cannot use changed mapping');
select set_config('test.op2',gen_random_uuid()::text,true);
select propose_operation(current_setting('test.tenant')::uuid,current_setting('test.op2')::uuid,'recordZettlePurchase',(select p from payload),'zettle-fixture',now()+interval '1 day');
set local "request.jwt.claims"='{"sub":"f0000000-0000-4000-8000-000000000451","role":"authenticated"}';
select decide_operation(current_setting('test.tenant')::uuid,gen_random_uuid(),current_setting('test.op2')::uuid,'approved','Checked whole purchase');
select is((select outcome from operation_decisions where operation_id=current_setting('test.op2')::uuid),'executed','other staff approves');
select is((select count(*) from sales),1::bigint,'one whole sale');
select is((select count(*) from sale_lines),2::bigint,'both lines atomically recorded');
select is((select provider from sales),'zettle','provider retained');
select is((select external_id from sales),'12345678-1234-4234-8234-123456789abc','purchase UUID retained');
select is((select sum(amount_ore) from seller_ledger_entries),10000::numeric,'existing engine computes seller credit');
select set_config('test.op3',gen_random_uuid()::text,true);
select propose_operation(current_setting('test.tenant')::uuid,current_setting('test.op3')::uuid,'recordZettlePurchase',(select p from payload),'zettle-fixture',now()+interval '1 day');
set local "request.jwt.claims"='{"sub":"f0000000-0000-4000-8000-000000000452","role":"authenticated"}';
select decide_operation(current_setting('test.tenant')::uuid,gen_random_uuid(),current_setting('test.op3')::uuid,'approved','Replay');
select is((select count(*) from sales),1::bigint,'another proposal cannot duplicate sale');
select is((select count(*) from seller_ledger_entries),1::bigint,'no double seller credit');
...
select lives_ok($$select propose_operation(current_setting('test.tenant')::uuid,gen_random_uuid(),'recordZettlePurchase',jsonb_build_object('importId',current_setting('test.collision'),'mappingRevision',1),'zettle-fixture',now()+interval '1 day')$$,'explicit collision resolution is usable');
...
select throws_like($$select resolve_zettle_line(current_setting('test.tenant')::uuid,gen_random_uuid(),current_setting('test.collision')::uuid,1,1,'abcdef00-0000-4000-8000-000000000003')$$,'%ITEM_NOT_FOUND%','cannot map item from another store');
reset role;
