...
select is((select status||'|'||amount_ore||'|'||request_source||'|'||approved_by from payouts where id=md5(current_setting('test.batch')||':'||current_setting('test.a'))::uuid),'approved|'||current_setting('test.avail_a')||'|staff|f0000000-0000-4000-8000-000000000311','payout derived from the batch, approved by the caller');
select is((select reason from payout_events where payout_id=md5(current_setting('test.batch')||':'||current_setting('test.a'))::uuid and kind='approved'),'September settlement','approval event carries the batch reason');
...
select is((select status||'|'||approved_by from payouts where id=md5(current_setting('test.op')||':'||current_setting('test.c'))::uuid),'approved|f0000000-0000-4000-8000-000000000312','C approved by the approver, not the proposer');
