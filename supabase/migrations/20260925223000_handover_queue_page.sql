-- Complete staff browsing; keep the legacy capped read during rolling deployment.
create function public.handover_queue_page(p_tenant uuid,p_query text default '',p_status text default 'all',p_offset integer default 0) returns jsonb
...
begin
 ...
 with matching as materialized (
  ...
  where h.tenant_id=p_tenant and (p_status='all' or h.status=p_status)
   and (q='' or strpos(lower(s.name),q)>0 or strpos('h-'||h.reference,q)>0)
 ), page as (
