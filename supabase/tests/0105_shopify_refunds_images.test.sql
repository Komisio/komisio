...
select publish_reception_review(current_setting('test.tenant')::uuid,gen_random_uuid(),current_setting('test.session')::uuid,1,null,current_setting('test.agreement')::uuid,jsonb_build_object('metadata',jsonb_build_object('description',jsonb_build_object('value','Test jacket','sourceIds',jsonb_build_array(current_setting('test.photo')),'certainty','observed')),'price',jsonb_build_object('currency','SEK','amount','100.00','rationale','Test','sourceIds',jsonb_build_array('f0000000-0000-4000-8000-000000000938'::text)),'questions','[]'::jsonb),now()+interval '1 day');
