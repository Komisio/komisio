...
select is((select suggestions->'metadata'->'description'->>'value' from reception_reviews where session_id=current_setting('test.session')::uuid),'Blue jacket','description published');
select is((select suggestions->'metadata' ? 'brand' from reception_reviews where session_id=current_setting('test.session')::uuid),false,'blank facts are left out');
