-- Paged directory read with the original RPC retained for tools and clients.
create function public.items_overview_page(p_tenant uuid,p_query text,p_stage text,p_limit integer,p_offset integer) returns jsonb
language plpgsql stable security definer set search_path='' as $$
declare q text:=coalesce(trim(p_query),''); n integer:=least(greatest(coalesce(p_limit,50),1),100); skip integer:=coalesce(p_offset,0); pattern text; rows jsonb; total integer;
begin
 ...
 select count(*)::int,
  ...
  ) order by s.accepted_at desc,s.id) from (select * from scan order by accepted_at desc,id limit n offset skip) s),'[]'::jsonb)
 into total,rows from scan;
 return jsonb_build_object('currency',komisio_private.store_currency(p_tenant),'items',rows,'total',total,'limit',n,'offset',skip,'query',nullif(q,''),'stage',p_stage);
end $$;
revoke all on function public.items_overview_page(uuid,text,text,integer,integer) from public,anon;
grant execute on function public.items_overview_page(uuid,text,text,integer,integer) to authenticated;

create or replace function public.items_overview(p_tenant uuid,p_query text,p_stage text,p_limit integer) returns jsonb
language sql stable security invoker set search_path='' as $$
 select public.items_overview_page(p_tenant,p_query,p_stage,p_limit,0)-'offset'
$$;
