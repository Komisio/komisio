...
select throws_like($$select queue_seller_communication(current_setting('test.tenant')::uuid,current_setting('test.c1')::uuid,current_setting('test.seller')::uuid,'message','message','v1','de','Hej','Text','none',null)$$,'%INVALID_INPUT%','unknown locale rejected');
