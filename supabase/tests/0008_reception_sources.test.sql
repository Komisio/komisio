...
select throws_like($$select save_reception_sources(current_setting('test.a')::uuid,gen_random_uuid(),current_setting('test.session')::uuid,1,jsonb_set(current_setting('test.sources')::jsonb,'{0,kind}','"photo"'))$$,'%INVALID_INPUT%','photo references require future protected capture');
