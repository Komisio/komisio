...
select is((select coalesce(array_agg(c.relname::text order by c.relname),'{}') from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='public' and c.relkind='r' and not exists(select 1 from pg_policy where polrelid=c.oid)),array['connector_calls','connector_clients','connector_codes','connector_functions','connector_grants','connector_tokens','statement_counters'],'only the engine-only tables (connector secrets and the counter) have no policy');
select is((select coalesce(array_agg(p.proname::text order by p.proname),'{}') from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and has_function_privilege('anon',p.oid,'execute') and not exists(select 1 from pg_depend d where d.objid=p.oid and d.deptype='e')),
 array['connector_call','connector_client','connector_token_info','exchange_connector_code','public_store_profile','reception_photo_access','refresh_connector_token','register_connector_client','seller_reception_photo_access','void_connector_secret'],'anonymous callers execute only the public profile read, the storage access hooks and the connector token functions (hash-gated, docs/HOSTED-MCP.md)');
