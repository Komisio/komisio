-- P2 S15 staged payout kinds: an agent may propose approving a requested
-- payout (reserves the amount) or marking an approved payout paid (reference
-- required). Both medium: a second person approves; execution runs the
-- ordinary transition as that person, with the operation id as the event id.
alter table public.pending_operations drop constraint pending_operations_kind_check;
alter table public.pending_operations add constraint pending_operations_kind_check check(kind in ('publishReceptionReview','saveInspectionDraft','acceptItem','recordReturn','adjustLedger','applyMarkdownBatch','bulkItemUpdate','approvePayout','markPayoutPaid'));
...
create function komisio_private.op_validate_approve_payout(value jsonb) returns boolean
language plpgsql immutable set search_path='' as $$
begin
 if value is null or jsonb_typeof(value)<>'object' then return false; end if;
 if not(value ?& array['payoutId','reason']) or (value-array['payoutId','reason'])<>'{}'::jsonb then return false; end if;
 if jsonb_typeof(value->'payoutId')<>'string' or (value->>'payoutId') !~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' then return false; end if;
 if jsonb_typeof(value->'reason')<>'string' or length(value->>'reason')>500 then return false; end if;
 return true;
end $$;
create function komisio_private.op_validate_mark_payout_paid(value jsonb) returns boolean
language plpgsql immutable set search_path='' as $$
begin
 if value is null or jsonb_typeof(value)<>'object' then return false; end if;
 if not(value ?& array['payoutId','reference','reason']) or (value-array['payoutId','reference','reason'])<>'{}'::jsonb then return false; end if;
 if jsonb_typeof(value->'payoutId')<>'string' or (value->>'payoutId') !~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' then return false; end if;
 if jsonb_typeof(value->'reference')<>'string' or length(trim(value->>'reference')) not between 1 and 200 then return false; end if;
 if jsonb_typeof(value->'reason')<>'string' or length(value->>'reason')>500 then return false; end if;
 return true;
end $$;
create function komisio_private.op_preflight_payout(p_tenant uuid,p_payload jsonb,p_expected text) returns jsonb
language plpgsql security definer set search_path='' as $$
declare payout public.payouts;
begin
 select * into payout from public.payouts where tenant_id=p_tenant and id=(p_payload->>'payoutId')::uuid;
 if not found then raise exception 'PAYOUT_NOT_FOUND'; end if;
 if payout.status<>p_expected then
  if p_expected='requested' then raise exception 'PAYOUT_NOT_REQUESTED'; else raise exception 'PAYOUT_NOT_APPROVED'; end if;
 end if;
 return jsonb_build_object('payout_id',payout.id,'seller_id',payout.seller_id,'amount_ore',payout.amount_ore,'status',payout.status);
end $$;
create function komisio_private.op_execute_approve_payout(p_tenant uuid,p_operation uuid,p_payload jsonb) returns uuid
language plpgsql security definer set search_path='' as $$
begin
 return public.approve_payout(p_tenant,p_operation,(p_payload->>'payoutId')::uuid,p_payload->>'reason');
end $$;
create function komisio_private.op_execute_mark_payout_paid(p_tenant uuid,p_operation uuid,p_payload jsonb) returns uuid
language plpgsql security definer set search_path='' as $$
begin
 return public.mark_payout_paid(p_tenant,p_operation,(p_payload->>'payoutId')::uuid,p_payload->>'reference',p_payload->>'reason');
end $$;
revoke all on function komisio_private.op_validate_approve_payout(jsonb),komisio_private.op_validate_mark_payout_paid(jsonb),komisio_private.op_preflight_payout(uuid,jsonb,text),
 komisio_private.op_execute_approve_payout(uuid,uuid,jsonb),komisio_private.op_execute_mark_payout_paid(uuid,uuid,jsonb) from public,anon,authenticated;
...
begin
 ...
 if (p_kind is not null and p_kind not in ('publishReceptionReview','saveInspectionDraft','acceptItem','recordReturn','adjustLedger','applyMarkdownBatch','bulkItemUpdate','approvePayout','markPayoutPaid'))
