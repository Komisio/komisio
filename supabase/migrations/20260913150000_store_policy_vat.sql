-- P2 S10: VAT modes as tenant policy keys (docs/VAT-CASES.md). Optional so every
-- published version stays valid; absent means the tenant has not chosen and no
-- sale can be recorded (VAT_MODE_NOT_SET in S11). Rate absent means 25.00.
...
begin
 ...
 optional:=array['vatModeConsignmentPrivate','vatModeStoreOwned','vatRatePercent'];
