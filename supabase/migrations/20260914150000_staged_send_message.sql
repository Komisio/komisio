-- P2 S18 staged kind sendMessage (low): an agent proposes the free-text block
-- of the general seller message template for one seller. The payload holds
-- only the seller, the locale and the free text; the template around it is
-- code and never part of the proposal. Preflight requires a seller with an
-- e-mail address. Execution records the approval; the application then
-- renders, queues and sends through the ordinary communication path with the
-- operation id as the communication id, so an approved message is sent once.
alter table public.pending_operations drop constraint pending_operations_kind_check;
alter table public.pending_operations add constraint pending_operations_kind_check check(kind in ('publishReceptionReview','saveInspectionDraft','acceptItem','recordReturn','adjustLedger','applyMarkdownBatch','bulkItemUpdate','approvePayout','markPayoutPaid','sendMessage'));
...
create function komisio_private.op_validate_send_message(value jsonb) returns boolean
language plpgsql immutable set search_path='' as $$
begin
 ...
 if jsonb_typeof(value->'locale')<>'string' or value->>'locale' not in ('sv','en') then return false; end if;
 if jsonb_typeof(value->'freeText')<>'string' or length(trim(value->>'freeText')) not between 1 and 1000 then return false; end if;
 return true;
end $$;
create function komisio_private.op_preflight_send_message(p_tenant uuid,p_payload jsonb) returns jsonb
language plpgsql security definer set search_path='' as $$
declare seller public.sellers;
begin
 select * into seller from public.sellers where tenant_id=p_tenant and id=(p_payload->>'sellerId')::uuid;
 ...
 return jsonb_build_object('seller_id',seller.id,'locale',p_payload->>'locale');
end $$;
-- Approval is the fact; the send is the application's follow-up keyed by the operation id.
create function komisio_private.op_execute_send_message(p_tenant uuid,p_operation uuid,p_payload jsonb) returns uuid
language plpgsql security definer set search_path='' as $$
begin
 if not exists(select 1 from public.sellers where tenant_id=p_tenant and id=(p_payload->>'sellerId')::uuid and email<>'') then raise exception 'SELLER_EMAIL_MISSING'; end if;
 return p_operation;
end $$;
revoke all on function komisio_private.op_validate_send_message(jsonb),komisio_private.op_preflight_send_message(uuid,jsonb),komisio_private.op_execute_send_message(uuid,uuid,jsonb) from public,anon,authenticated;
...
begin
 ...
 if (p_kind is not null and p_kind not in ('publishReceptionReview','saveInspectionDraft','acceptItem','recordReturn','adjustLedger','applyMarkdownBatch','bulkItemUpdate','approvePayout','markPayoutPaid','sendMessage'))
