# Functional roadmap: what the new Komisio must do

Status: proposal for owner review, 2026-09-12. Written from a full read of the
three earlier repositories (back office with Angular UI and ~700 C# classes,
the Expo mobile app with 60 screens, and the Windows print service). This is a
business-analysis document: it lists the functionality the earlier system
provided, decides for each piece whether the new Komisio recreates it,
transforms it, or drops it, and orders the work into phases. It does not
decide schema, commission or VAT rules; those remain in
[open questions](open-questions.md). No code, data model or text from the
earlier system is copied; entity and screen names are used only to identify
what is meant.
...
1. **Most screens were registers.** Categories, products, dimensions, labels,
   ...
   structure so that the system could classify things. A vision model does
   that classification better from a photo than a person does from a dropdown.
2. **The financial core was thin but real.** Sale with lines, VAT breakdown by
   ...
   statements, daily transfer, Fortnox voucher. This is the part that must be
   rebuilt exactly, with tests, not reimagined.
...
- **No registers.** No category tree, product catalogue, dimension list or VAT
  rate table to maintain. A garment is described by the model from photos and
  evidence; the store confirms. Where an integration needs a taxonomy (POS
  categories, Shopify collections), the adapter maps at the edge from the
  description, and the mapping is a learned, reviewable table, not a register.
- **GUI only where a person must decide or must be shown something.** The
  minimum web surface is: approve queue, receive, sell/return (via POS
  integration), payouts, statements, settings. Everything else is an agent
  conversation over the same engine, or a report the system writes.
- **One engine, four callers.** Web, mobile, MCP agents and integrations call
  the same operations. Nothing has its own write path.
- **Recreate 95 percent of the functional outcomes, not 95 percent of the
  screens.** Section 9 accounts for every capability.
...
from section 7. "Done" and "Partial" refer to the current repository.

### 3.1 Platform, tenancy and access

| Earlier capability | New Komisio | Form | Phase |
| --- | --- | --- | --- |
| Tenants as stores, host administration, tenant approval for the mobile app | Tenant is the store; chain grouping later; no host UI, host operations are scripts and agent tools | Engine | Done |
| Users, roles (ABP identity), MFA policy per tenant (off/recommend/enforce), remembered devices | Owner/admin/staff/readonly, invitations, TOTP MFA enforced in DB when enrolled; per-tenant MFA policy | Engine, UI | Done (policy: P2) |
| Tenant user agreement with translations, acceptance per user, "must accept before submitting" | Versioned seller agreement, staff-recorded evidence; seller-authenticated acceptance on mobile review | Engine, UI | Done (auth acceptance: P1) |
...
| Billing: plans Free/Silver/Gold/Platinum, AI plans Base/Lite/Pro with unit quotas, Stripe checkout and portal, invoices, host overview | One free tier and one hosted tier (decided); Stripe subscription for the hosted tier; AI usage metered per tenant with a quota; no invoice generation in core | Integration | P6 |
| Host tenant dashboard (GMV, integration health, risk score) | Report generated for the operator, not a screen | Report | P6 |
| Tenant backups and restore (snapshots, restore jobs, SLA document) | Platform backup/restore procedure (documented, exercised); per-tenant export as a file | Ops, Engine | P5 |
...
| Earlier capability | New Komisio | Form | Phase |
| --- | --- | --- | --- |
| Seller record: name, contact, address, SSN, commission rate and override, notes, active flag | Seller as contact (done); identity strengthened by verified e-mail (done), BankID later; commission terms per seller as tenant policy override | Engine | Done (terms: P2) |
...
| Seller data export (GDPR) | Export operation producing a file per seller | Engine | P5 |
...
| Staff item registration on mobile: photos, vision analysis, title, description, price, category, product, dimensions, status, print label; barcode scanner to open an item | Single-garment reception (done): photos, evidence, model proposal, staff review, seller approval. Mobile staff surface reuses it | Engine, UI | Done (mobile staff UI: P1) |
| Smart intake (staff web and seller mobile): images → draft item with name, description, price, matched product, dimensions, duplicates, confidence, "why" explanation; edit; submit | Reception with AI assistance (done) plus per-fact confirmation (T4) and duplicate check (see 3.4) | Engine, UI | Partial → P1 |
...
| Categories, products (with VAT and external mappings), dimensions (canonical: brand, color, size, material, pattern, season, style, gender, age group), item dimensions, auto-create flags | Dropped as registers. The model supplies the same fields as facts with source citations (brand, color, size, material, condition exist today); VAT class is derived at sale from ownership and item kind; integration mappings live in the adapter | Engine, Agent | Done (fields) |
...
| Item journey: events (created, status, price, listed, sold, donated, returned, synced, label printed, AI suggested/accepted/corrected, duplicate dismissed) | Item events are the audit stream; AI provenance events included (T4) | Engine | P1 |
...
| Payout rails: Swish payout (instruction, callback, retry), PayPal payout (batch, item, unclaimed), bank (manual with reference) | Manual first (decided); Swish and Stripe as adapters (decided candidates); PayPal on demand | Integration | P3 |
...
| Charge: pending/settled/cancelled/reserved for payout; collection modes POS-only or integrated; Swish and Stripe payment start and confirm; external payment; debt limit per seller | Charge as ledger entries; collection through the same payout rails in reverse; debt limit as policy | Engine, Integration | P4 |
...
| Accounted as export target | Same extension contract | Integration | P3 |

### 3.9 AI layer

| Earlier capability | New Komisio | Form | Phase |
| --- | --- | --- | --- |
| Unified LLM client with providers, retries, timeouts, circuit breaker, kill switch per tenant, PII sanitizer | Assistance adapters behind one port (done for reception); provider choice per feature; tenant allowlist becomes tenant policy; sanitizer before any external call | Engine | Partial → P1 |
...
| Prompt registry: versioned, per-locale, draft/active/retired, placeholders, rollback, admin UI | Prompts are versioned code with a pinned hash (done); per-tenant tone and locale as placeholders; no admin UI | Engine | Done |
...
| Vision service: image → type, condition 1–5, price suggestion, tags, dimensions | Done as sourced suggestions; condition scale and tags added to the fact contract | Engine | Done |
...
| AI agent log (action, input, output, confidence, escalation) | Operations and access events (done) | Engine | Done |
...
| --- | --- | --- |
| Category, product, dimension registers and their admin UIs, help pages and quick-start generation | The model classifies from evidence; registers were maintenance without value | Sourced facts on the item; adapter-side mapping for integrations |
| VAT rate register per country | Sweden first; VAT treatment is a frozen per-line decision, rates are policy constants | Tenant policy |
...
| Translations admin, tenant info page CMS, agreement translations | Model-generated per tenant on read; agreements stay versioned text (done) | 3.1 |
...
new Komisio has one: an agent talks to the engine through MCP, with read tools
and staged-write tools, and every surface (web copilot, mobile assistant,
voice, scheduled jobs) is a client of that one contract. The tool catalogue,
...
| Intake | list receptions, read reception, read history, read photo, duplicate check, identify by image | propose review (done), propose batch reception, propose acceptance |
...
the model; reads carry `evidenceIsUntrusted`; writes return
`staged:true, executed:false`; risk level derives from the kind; medium and
high kinds need a different approver than the proposing session; owner-enabled
auto-execution scopes are the only path to unattended writes. The earlier
system's PII sanitizer becomes a mandatory step in every adapter that sends
text outside the tenant.
...
Each phase ends with something a store can use. Phases follow the open
questions: nothing financial is built before its question is answered.

| Phase | Theme | Slices | Needs from owner |
| --- | --- | --- | --- |
| P0 (done) | Foundation and reception | Tenancy, users, MFA, agreements, bag receipts, inspection drafts, single-garment reception with AI, seller mobile review, staged operations, MCP reads and propose | – |
...
| P2 | Sell and settle | Zettle sales pull, sale lines with VAT freeze, returns, seller ledger and balance, payout request/approve/pay (manual rail), statements, labels and local print agent, lifecycle queue, batch reception, duplicate check, notifications, communication log, usage metering, staff agent tools (analytics, price proposals), bulk staged ops | Questions 5, 7, 8, 10 |
| P3 | Run the store | Markdown agent with policy, 100-hours lifecycle and price feed, day close and Fortnox export, Swish and Stripe payout rails, settlement batch, insights brief, visual pricing evidence, Shopify adapter, self drop-off handover with QR, store profile | Questions 4, 6, 9, 11 |
...
| P5 | Open up | Partner REST subset with OAuth clients, import wizard as staged op, kiosk integration contract, voice adapter, public buyer assistant, backup and restore exercise, seller data export, quality and fraud agents | – |
...
- Does the 100-hours concept belong in the core as a lifecycle policy, or is
  it one store's configuration of a generic decay policy?
...
- Is a store-owned (purchased) item in scope for the first sale slice, or
  consignment only?
- Which POS is first: Zettle (earlier live path) or Shopify POS?
...
Eighty of 81 rows, 98 percent, keep their outcome. The registers and admin
