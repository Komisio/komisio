-- Extend the existing staged mechanism; no new tables or commercial meaning.
-- Previous applied migrations remain immutable. Existing reception behavior is preserved.
alter table public.pending_operations drop constraint pending_operations_kind_check;
alter table public.pending_operations add constraint pending_operations_kind_check check(kind in ('publishReceptionReview','saveInspectionDraft'));
...
begin
 ...
 begin
  ...
  elsif op.kind='saveInspectionDraft' then
   result:=public.save_inspection_draft(p_tenant,p_operation,(op.payload->>'bagId')::uuid,(op.payload->>'draftId')::uuid,
    (op.payload->>'expectedRevision')::integer,op.payload#>>'{fields,description}',op.payload#>>'{fields,category}',op.payload#>>'{fields,condition}');
  else
...
revoke all on function komisio_private.valid_operation_payload(text,jsonb) from public,anon,authenticated;
