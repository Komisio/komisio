...
select throws_ok($$select reserve_reception_assistance(current_setting('test.b')::uuid,gen_random_uuid(),current_setting('test.session')::uuid,1,'test-model','reception-batch-v4')$$,'P0001','RECEPTION_CHANGED','cross store session denied');
...
select is(reserve_reception_assistance(current_setting('test.a')::uuid,current_setting('test.request')::uuid,current_setting('test.session')::uuid,1,'test-model','reception-batch-v4'),true,'batch attempt reserved');
...
select is(reserve_reception_assistance(current_setting('test.a')::uuid,current_setting('test.request')::uuid,current_setting('test.session')::uuid,1,'test-model','reception-batch-v4'),false,'same request never invokes twice');
select throws_ok($$select reserve_reception_assistance(current_setting('test.a')::uuid,current_setting('test.request')::uuid,current_setting('test.session')::uuid,1,'different','reception-batch-v4')$$,'P0001','REQUEST_CONFLICT','payload binding');
select throws_ok($$select reserve_reception_assistance(current_setting('test.a')::uuid,gen_random_uuid(),current_setting('test.session')::uuid,2,'test-model','reception-batch-v4')$$,'P0001','RECEPTION_CHANGED','stale source denied');
select throws_ok($$select reserve_reception_assistance(current_setting('test.a')::uuid,gen_random_uuid(),current_setting('test.session')::uuid,1,'test-model','reception-batch-v4')$$,'P0001','ASSISTANCE_LIMIT','cooldown serializes new attempts');
...
select gen_random_uuid(),current_setting('test.a')::uuid,current_setting('test.session')::uuid,1,'89000000-0000-4000-8000-000000000001','test-model','reception-batch-v4',now()-interval '1 hour' from generate_series(1,10);
...
set local role authenticated;
select throws_ok($$select reserve_reception_assistance(current_setting('test.a')::uuid,gen_random_uuid(),current_setting('test.session')::uuid,1,'test-model','reception-batch-v4')$$,'P0001','ASSISTANCE_LIMIT','ten attempts in rolling day is hard cap');
reset role;
...
select throws_ok($$select reserve_reception_assistance(current_setting('test.a')::uuid,gen_random_uuid(),current_setting('test.session')::uuid,1,'test-model','reception-batch-v4')$$,'42501','FORBIDDEN','readonly cannot spend');
...
select throws_ok($$select reserve_reception_assistance(current_setting('test.a')::uuid,gen_random_uuid(),current_setting('test.session')::uuid,1,'test-model','reception-batch-v4')$$,'42501',null,'unverified/nonmember rejected');
reset role;
...
select throws_ok($$select reserve_reception_assistance(current_setting('test.a')::uuid,gen_random_uuid(),current_setting('test.session')::uuid,1,'test-model','reception-batch-v4')$$,'42501',null,'MFA blocks reservation');
reset role;
...
set local role authenticated;
set local "request.jwt.claims"='{"sub":"89000000-0000-4000-8000-000000000002","role":"authenticated"}';
select throws_ok($$select reserve_reception_assistance(current_setting('test.a')::uuid,current_setting('test.other_request')::uuid,current_setting('test.session')::uuid,1,'test-model','reception-batch-v4')$$,'P0001','REQUEST_CONFLICT','another staff actor cannot reuse original request');
select * from finish();
rollback;
