-- A lower ceiling while a store is new (2026-09-16, Opus). Registration is
-- open: a confirmed e-mail address is the only thing between a stranger and a
-- store, and a store may write to any address its staff type in. The rolling
-- day caps were set high enough never to inconvenience a real shop, which left
-- a fresh throwaway store able to send a thousand messages from the
-- deployment's verified sending domain on its first day. The money is not the
-- risk; the sending reputation is. A domain that lands on a blocklist stops
-- every real store's seller notices at once.
--
-- So the cap depends on how long the store has existed. A store younger than
-- email_trust_days gets the new-store numbers, and the established numbers
-- after that. A real shop is unaffected: on its first week it has a handful of
-- sellers, not hundreds. All five numbers belong to the host, under Plattform,
-- and no store can change its own.
alter table public.platform_settings
 add column email_new_cap integer not null default 50 check(email_new_cap>=0),
 add column invite_new_cap integer not null default 5 check(invite_new_cap>=0),
 add column email_trust_days integer not null default 7 check(email_trust_days>=0);
...
begin
 ...
 if (select count(*) from public.seller_communications c where c.tenant_id=p_tenant and c.queued_at>now()-interval '1 day')
  >=(select case when t.created_at>now()-make_interval(days=>s.email_trust_days) then s.email_new_cap else s.email_daily_cap end from public.platform_settings s,public.tenants t where t.id=p_tenant) then raise exception 'EMAIL_DAILY_CAP' using errcode='55000'; end if;
...
begin
 ...
 if (select count(*) from public.tenant_invitations v where v.tenant_id=p_tenant and v.created_at>now()-interval '1 day')
  >=(select case when t.created_at>now()-make_interval(days=>s.email_trust_days) then s.invite_new_cap else s.invite_daily_cap end from public.platform_settings s,public.tenants t where t.id=p_tenant) then raise exception 'INVITE_DAILY_CAP' using errcode='55000'; end if;
...
begin
 ...
 return jsonb_build_object('enabled',s.ai_credits_enabled,'monthlyCapOre',s.ai_monthly_cap_ore,'includedOre',s.ai_included_ore,'packOre',s.ai_pack_ore,
  ...
  'connectorDailyCap',s.connector_daily_cap,'emailNewCap',s.email_new_cap,'inviteNewCap',s.invite_new_cap,'emailTrustDays',s.email_trust_days,'emailDailyCap',s.email_daily_cap,'inviteDailyCap',s.invite_daily_cap,
...
begin
 ...
 update public.platform_settings set
  ...
  email_new_cap=coalesce((p->>'emailNewCap')::integer,email_new_cap),
  invite_new_cap=coalesce((p->>'inviteNewCap')::integer,invite_new_cap),
  email_trust_days=coalesce((p->>'emailTrustDays')::integer,email_trust_days),
