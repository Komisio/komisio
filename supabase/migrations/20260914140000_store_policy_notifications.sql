-- P2 S18: the store opts in to automatic seller notifications with the
-- optional boolean automaticSellerNotifications. Absent means off. Every
-- automatic message is the same logged, template-bound communication a person
-- could send, keyed by the fact so it is queued once. Validator replaced
-- additively; old versions stay valid.
...
begin
 ...
 optional:=array['vatModeConsignmentPrivate','vatModeStoreOwned','vatRatePercent','assistanceEnabled','assistanceMonthlyQuota','automaticSellerNotifications'];
