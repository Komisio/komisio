...
select set_config('test.suggestions','{"attributes":[{"slug":"description","definitionVersion":1,"value":"Blue jacket","sourceIds":["c0000000-0000-4000-8000-000000000010"],"certainty":"observed"}],"price":{"currency":"SEK","amount":"250.00","rationale":"TEST","sourceIds":["c0000000-0000-4000-8000-000000000011"]},"questions":[]}',true);
reset role;
...
select throws_like($$select propose_operation(current_setting('test.tenant')::uuid,gen_random_uuid(),'publishReceptionReview',jsonb_set(current_setting('test.payload')::jsonb,'{suggestions,attributes,0,sourceIds}','["c0000000-0000-4000-8000-000000000099"]'),'test-agent',now()+interval '1 day')$$,'%RECEPTION_UNKNOWN_SOURCE%','unknown source rejected at proposal');
...
select throws_like($$select propose_operation(current_setting('test.tenant')::uuid,gen_random_uuid(),'publishReceptionReview',jsonb_set(current_setting('test.payload')::jsonb,'{suggestions,attributes,0,certainty}','"tentative"'),'test-agent',now()+interval '1 day')$$,'%INVALID_INPUT%','tentative facts cannot be proposed for publication');
